...
| N5 | Full suite: zero network, zero credentials | CI (`.github/workflows/ci.yml`) never sets `DO_INFERENCE_KEY`. **Incident, fixed this session**: `tests/conftest.py` originally used `monkeypatch.delenv("DO_INFERENCE_KEY")`, which does not stop `pydantic-settings` from reading a real key out of an actual `.env` file (a lower-priority but still-consulted source) — once a real `.env` existed for the live runs below, ~11 test-suite job runs made real API calls before this was caught. Fixed to `monkeypatch.setenv("DO_INFERENCE_KEY", "")`, which does take priority over the dotenv file; verified with `.env` present that a full suite run creates zero new spend-ledger files. See `BUILD_LOG.md` for the full incident writeup. |
